...
import com.GroceryOrderingSystem.server.Server_app;
import com.GroceryOrderingSystem.server.Server_app2;
import com.GroceryOrderingSystem.server.Server_app3;
...
public class Key_Store_Int_Imp extends UnicastRemoteObject implements Key_Store_Int, Runnable {
    ...
    private Thread th;
    public Store store_obj = Store.getInstance();
...
//        lgm.log(Level.INFO, "Thread started...");
    ...
    public ArrayList remote_put_pair(ArrayList request, int serverno) throws Exception {
        ServerNo = serverno;
    ...
    public synchronized ArrayList put_pair(ArrayList messages){
        ...
        if (!retrieved.containsKey((String) messages.get(1))) {
            ...
            if (ServerNo == 1) {
                Server_app.checkMart(store_obj);
            }
            else if (ServerNo == 2) {
                Server_app2.checkMart(store_obj);
            }
            else if (ServerNo == 3) {
                Server_app3.checkMart(store_obj);
            }
